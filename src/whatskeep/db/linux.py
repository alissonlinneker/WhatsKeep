"""Linux WhatsApp Desktop database reader."""
...
class LinuxDBReader(BaseDBReader):
    """Linux WhatsApp DB reader.

    Attempts multiple known installation paths.  Falls back gracefully
    ...
    def __init__(self) -> None:
        ...
        home = Path.home()
        self._known_paths: list[Path] = [
            # Snap package
            home / "snap" / "whatsapp-for-linux" / "current" / ".config" / "WhatsApp",
            # Flatpak (nickvision wrapper)
            home / ".var" / "app" / "io.github.nickvision.Application" / "data" / "WhatsApp",
            # Generic / manual install
            home / ".config" / "WhatsApp",
            # whatsapp-for-linux native
            home / ".config" / "whatsapp-for-linux",
        ]
            ...
            for pattern in ("*.db", "*.sqlite"):
                for candidate in base.rglob(pattern):
                    if candidate.is_file():
                        return candidate
        logger.warning(
            "Linux WhatsApp database not found in any known location"
        )
        return None
    ...
    def get_media_records(self) -> list[DBMediaRecord]:
        ...
        if path is None:
            logger.warning(
                "No Linux WhatsApp database available — returning empty list"
            )
            return []
        ...
        try:
            ...
            logger.warning(
                "Cannot open Linux WhatsApp database at {}: {}", path, exc
            )
            return []
    ...
    def _try_read_media(self) -> list[DBMediaRecord]:
        ...
        try:
            ...
            logger.warning(
                "Failed to list tables in Linux WhatsApp DB: {}", exc
            )
            return []
        ...
        logger.warning(
            "Linux WhatsApp database does not contain recognised "
    ...
    def _execute_and_map(self, query: str) -> list[DBMediaRecord]:
        ...
        try:
            ...
            logger.warning(
                "Failed to query Linux WhatsApp database: {}", exc
            )
            return []
        ...
        logger.info(
            "Linux WhatsApp DB: found {} media records", len(records)
        )
        return records
